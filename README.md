...
- 초기 방식의 한계를 확인하고 건설안전지침 PDF 기반으로 방향을 전환하면서, 검색에 사용할 근거 데이터를 어떻게 구성하는지도 중요하다고 느낌
- Query Expansion, Reranking, 2단계 답변 생성을 적용했지만, Query → 검색 문서 → 개별 답변 → 최종 대책의 각 단계가 원래 사고 상황을 적절히 반영하는지 평가하지 못한 점이 아쉬움
